...
plot1 <- function() {
    plot (li$x,li$t*1e6, col=liCol, main="SerialiseData", xlab="size", ylab="time (µs)",ylim=c(0,9e5))
...
plot2 <- function() {
    plot (li$x,li$t*1e6, col=liCol,main="SerialiseData", xlab="size", ylab="time (µs)",ylim=c(0,5e2))
    points(lb$x,lb$t*1e6,col=lbCol)
    points(lt$x,lt$t*1e6,col=ltCol)
    points(lm$x,lm$t*1e6,col=lmCol)
