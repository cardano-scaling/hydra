...
+ `address=$address` -> In the case of a `SnapshotConfirmed` message, it will be filtered out unless one of its `confirmed` transactions pays to the provided address. Note that a snapshot confirming no transaction at all, such as one that only settles a deposit or a decommit, therefore never reaches a client that gave an address. No other message is filtered: `SnapshotConfirmed` is the only server output carrying transactions to match an address against. An `address` given without a value, as in `?address` or `?address=`, is ignored rather than applied as a filter that matches nothing.
