...
- A rollback can erase several settlements at once. They are then re-posted one at a time, in version order: the chain only accepts the lowest one, because each settlement bumps the on-chain version by one. The node posts that lowest one at rollback time and posts the next only when it observes the previous one landing again. If the rollback erased no settled increment or decrement, but one was still in flight, that one is re-posted instead.
