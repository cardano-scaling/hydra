...
- When an increment or decrement transaction settles on-chain (`CommitFinalized`/`DecommitFinalized`), the node retains the signed snapshot that authorized it, keyed by the version it was based on, together with the slot the settlement was observed at. Retained snapshots are dropped once no rollback can reach them anymore. If a later rollback reaches past the observed slot, the settling transaction was erased from the chain and the node re-posts it from the retained snapshot. This also covers the case where newer snapshots were confirmed in the meantime (see [#2741](https://github.com/cardano-scaling/hydra/issues/2741)).
