...
- A companion document, [ADR-035 flows explained](./2026-08-05_035-flows-explained.md), walks both pipelines step by step, with data-provenance tables and a worked close.
...
1. **An exit request is an ordinary L2 transaction** that moves the owner's outputs into the exit queue, which is part of the snapshot state. Concretely, the transaction is ledger-valid on L2 and marks its exit outputs with a protocol-defined marker (a designated output-datum tag; the exact convention is fixed at specification time), preserving ledger isomorphism: the L2 ledger validates the transaction as any other, and the snapshot logic moves marked outputs out of `utxo` (and `A_active`) into the queue on confirmation. Entries get consecutive indices deterministically: snapshot-confirmation order of transactions, then output index within a transaction. Full L2 throughput, many exits per snapshot, nothing blocks. **Exits are final**, the same finality a rollup withdrawal has.
2. **The node checks each exit output at entry**: minimum ADA, `maxValSize`, and "a decrement paying just this output fits L1 limits". Because a decrement can pay any batch size down to one, this per-output check is complete *under the L1 protocol parameters in force at entry time*: if the output passes, some decrement can always pay it. Entry checks should also validate against any ratified-but-not-yet-enacted parameter update, which is visible on-chain before it takes effect; a parameter change can nonetheless make an already-queued entry unpayable, an accepted limitation analyzed in the security section (threat 13). (Today's `utxoToDecommit` set is validated nowhere.)
...
- **Multi-deposit increments**: certificates are per-deposit and increments commute, so one increment can spend the head output plus N deposit outputs (one certificate each). Batching is an optimization with a guaranteed one-at-a-time fallback. Its binding cost is signature verification rather than transaction size: k deposits mean k certificates of n signatures each, so the execution budget caps the batch at k × n verifications plus per-deposit message rebuilding. This wants benchmarking during specification; if the cap bites, certificate aggregation, one signing round over a *set* of deposit output references, composes with the exact-input-set rule and is a compatible follow-up optimization.
...
- Partial-fanout chunking is reused for distributing large states at fanout, now including the unabsorbed-deposit suffix replay and the exit-queue prefix; close and contest themselves are constant-size. Chunking the two sequential replays additionally requires replay cursors in the closed datum between chunks (see close, contest, and fanout above), which is new machinery on top of the reused chunking. Batched exit payment reuses the same prefix-chunking approach with transaction size as its only bound.
- The deposit lifecycle tracking and activation buffers are repurposed for certificate timing and the increment and exit-queue maturity buffers, which stay operator-configurable delays with conservative defaults (see rollbacks in the security section).
...
- **Exit payment is FIFO.** An exit waits behind earlier queue entries. Every entry is L1-valid by construction under the parameters at entry time and anyone can batch-pay the whole prefix, so absent an adverse L1 parameter change the queue can be delayed but never stalled; the parameter-change case is an accepted limitation with fanout as the backstop (threat 13).
...
- Deposit-to-L2 latency is comparable to today (deposit → certificate → increment → observation + maturity → next snapshot), with far less machinery. The maturity portion is an operator-set delay with a conservative default, so each operator trades settlement latency against rollback safety rather than accepting a protocol-fixed wait (see rollbacks in the security section).
- API reshape (sketch, to be detailed at implementation time): `POST /decommit` becomes submitting an L2 exit transaction (a ledger-valid transaction with marked exit outputs), and because exits are final the API surfaces irrevocability explicitly, with the server acknowledgement stating the exit cannot be cancelled; deposit lifecycle server outputs change (certificate round instead of commit-approval via snapshot); `Snapshot` drops `version`/`utxoToCommit`/`utxoToDecommit` and gains `absorbedDeposits` and the exit-queue view.
...
5. **Griefing the queue.** Flooding it with dust exits delays later withdrawals, but every entry costs the attacker a real min-ADA-bearing output that irrevocably leaves their own L2 balance, and batch decrements clear prefixes bounded only by transaction size, chaining across consecutive transactions. Parties may additionally rate-limit exit transactions per snapshot as policy. A single pathological output that no transaction could ever pay is rejected at entry: the per-output check is complete precisely because payment can always degrade to `k = 1`, under the parameters in force at entry time (for later parameter changes, see threat 13). (This also guarantees, for exit outputs, that fanout can always distribute them; L2-internal outputs whose size makes them undistributable remain a pre-existing limitation unchanged by this design.)
...
11. **Domain separation.** Exit entries (`H("exit" <> index <> output)`) and active outputs live in separate commitment structures (`Q_exit` and `A_active`), so a proof for one role cannot be replayed as the other by construction; the entry format additionally binds each output to its queue position and disambiguates identical outputs queued twice. The same discipline covers every hash construction: claim-chain steps are tagged `"claim"`, queue-chain steps `"queue"`, entry leaves `"exit"`, so no value of one role can ever equal a value of another, by input disjointness rather than by case analysis. Tags alone do not rule out *boundary* ambiguity where variable-length fields meet, and the exit leaf is the one preimage with attacker-influenced variable-length content, since the owner picks the output bytes: hence the fixed-width index and self-delimiting CBOR output encoding required in the on-chain data model, which make every leaf preimage parse uniquely and close the redirection attack where `(index, output)` and some `(index', output')` concatenate to the same bytes. The claim and queue chain steps need no such care: all their operands are fixed 32-byte values. The two multisigned message types, snapshot tuples `(headId, number, A_active, Q_exit, absorbedDeposits)` and claim certificates `(headId, depositOutRef, commitsHash)`, are signed under the same party keys and therefore carry distinct domain tags, so a signature over one can never verify as the other. Accumulator collision-resistance assumptions are unchanged from the current protocol.
...
13. **L1 protocol-parameter drift.** Exit entries are validated against the L1 protocol parameters in force when they enter the queue, and then wait an unbounded time. A subsequent adverse parameter change (a min-ADA increase, a `maxValSize` decrease, an execution-price change shrinking what fits one transaction) can make an already-queued entry unpayable by any decrement, and FIFO contiguity then stalls every entry behind it: the L1 payment path is dead until close. Funds stay safe, since fanout distributes the whole queue, subject to the same pre-existing limitation that fanout cannot create an output the new rules forbid outright, which affects all L2 outputs today, not only queue entries. This is an **accepted limitation**, deliberately left without a protocol mechanism: parameter updates are rare, governance-visible before enactment, and epoch-granular, so entry checks can validate against ratified-but-unenacted updates as hardening and parties can drain the queue before an adverse change takes effect; the residual extreme case is handled by closing the head. A skip-or-requeue mechanism that would let decrements pass over an unpayable entry was considered and rejected: it reintroduces a signing round on the settlement path and weakens the contiguity rule that makes double-pay structurally impossible.

Protocol invariants to carry into the specification: (i) snapshots reference only observed L1 state and grow-only L2 state; (ii) `claimedDeposits` is append-only while open and frozen at close; (iii) `exitsPaid` only grows and is written only from L1; (iv) an exit entry, once signed, keeps a stable index in all later snapshots until observed as paid; (v) the value-conservation identity of point 6; (vi) every `ReqSn` pins exactly one settlement-chain position, and all absorption effects (`absorbedDeposits` advance, deposited outputs entering `utxo`, pruning of queue entries below the observed `exitsPaid`) are deterministic functions of that position; absorption is prefix-only, so no increment can be skipped. A party that has not yet observed the chain up to the pinned position waits for its chain sync to catch up; a party that has observed it but whose maturity buffer has not yet elapsed rejects the `ReqSn` with that reason, and the leader re-requests later, so the delay is bounded by the most conservative party's buffer and is always visible, never a silent stall (leaders pin only positions matured past their own buffer plus a grace margin, making rejections rare). Observation skew between parties therefore never diverges state: settlement order is fixed by L1 (every settlement spends the head output), and lag only delays a signature. (vii) each increment appends exactly one chain entry per deposit-script input, and the set of deposit-script inputs equals the certified set, so every unit of head value that enters is recorded by exactly one append and no certificate authorizes more than one append. (viii) a signed snapshot's `absorbedDeposits` and committed queue front name only buffer-deep (rollback-safe) L1 positions; close and contest are constant-size, and the only state-proportional chain and queue replays happen at fanout, which is chunked, so no volume of unabsorbed deposits or queued exits can make a head uncloseable. (ix) every hash preimage in the protocol parses uniquely: domain tags are pairwise distinct, and every variable-length field is self-delimiting or fixed-width, so no two logical inputs share a byte-level preimage.
...
- Benchmarking increment certificate verification (k deposits × n signatures each) to size realistic deposit-batch caps, and evaluating set-level certificate aggregation if the cap bites.
- Specifying the rollback recovery of security consideration 7 ("re-sign from the last consistent snapshot") as its own state machine: how parties agree on the last consistent snapshot and how in-flight `ReqSn` state is discarded. This class of recovery is exactly where today's implementation has repeatedly gone wrong, so it deserves first-class treatment rather than a remark.
...
- A second review pass added four more points warranting focused attention: the L1 parameter-drift limitation on the exit queue (threat 13, accepted rather than mechanized), the unique-parse encoding requirement on the exit-leaf preimage (threat 11, invariant ix), the replay cursors that chunked fanout must carry between transactions (close section), and explicit reject-and-re-request semantics for a `ReqSn` pinning a not-yet-matured position (invariant vi).
