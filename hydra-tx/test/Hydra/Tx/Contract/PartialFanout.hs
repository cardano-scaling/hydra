...
fullAccumulator =
  Accumulator.buildFromUTxO @Tx healthyFullUTxO
...
duplicateFullAccumulator =
  Accumulator.buildFromUTxO @Tx duplicateFullUTxO
...
-- out of the pool backing everyone else's outputs and left the final step's
-- strict value equation unsatisfiable. With the closed datum committing to the
-- live set only, the pre-settled output is not a member and the step fails
