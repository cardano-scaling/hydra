...
-- An env whose deposit period AND activation are the modeled T_deposit (whole seconds); since #2802
-- the Inactive->Active transition keys off 'depositActivation' (Expired still off 'depositPeriod'),
-- so both must be tDep for the real tick to match the extracted 'depositStatusRef' (which models a
-- single T_deposit for both boundaries).
tickEnv :: Integer -> Environment
tickEnv tDep = aliceEnv{depositPeriod = fromInteger tDep, depositActivation = fromInteger tDep}
