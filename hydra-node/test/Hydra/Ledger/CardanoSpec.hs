...
import Hydra.Ledger.Cardano (CardanoEra, CardanoTx (..), CardanoTxWitnesses, cardanoLedger, genCardanoTx, genUtxo)
...
spec = do
  ...
  it "should parse a CardanoTx" $ do
    let bs =
          "{\"witnesses\":\
          \    {\"addresses\": [\"8200825820db995fe25169d141cab9bbba92baa01f9f2e1ece7df4cb2ac05190f37fcc1f9d58400599ccd0028389216631446cf0f9a4b095bbed03c25537595aa5a2e107e3704a55050c4ee5198a0aa9fc88007791ef9f3847cd96f3cb9a430d1c2d81c817480c\"],\
          \    \"scripts\": {}\
          \    },\
...
appliesValidTransaction =
  forAll genUtxo $ \utxo ->
    forAll (genCardanoTx utxo) $ \tx ->
      let result = applyTransactions cardanoLedger utxo [tx]
       in isRight result
            & counterexample ("Error: " <> show result)
            & counterexample ("JSON for tx: " <> unpack (decodeUtf8With lenientDecode $ toStrict $ Aeson.encode tx))
