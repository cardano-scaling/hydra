...
performRestartNode st party = do
  ...
  case (Map.lookup party stores, Map.lookup party threadsByParty, findHsk) of
    (Just eventStore, Just oldThread, Just hsk) -> do
      ...
      -- them before the next action: crashed again in the same instant, it
      -- would lose them for good, and a lost AckSn leaves the party unable to
      -- ever confirm that snapshot.
      lift $ threadDelay 1
      modify $ \n ->
        n
