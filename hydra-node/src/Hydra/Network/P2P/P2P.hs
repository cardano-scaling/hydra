...
import Cardano.Prelude (MVar, async, decodeUtf8, link, newEmptyMVar, putMVar, readMVar)
...
remoteServerAddr host tracer = withSocketsDo $ do
  ...
  case etransport of
    Left e -> traceWith tracer (P2PServerDisconnected $ "remoteServerAddr:Failed to create transport:" <> show e) >> fail "remoteServerAddr:Failed to create transport"
    Right transport -> do
      eendpoint <- newEndPoint transport
      case eendpoint of
        Left e -> traceWith tracer (P2PServerDisconnected $ "remoteServerAddr:Failed to create endpoint" <> show e) >> fail "remoteServerAddr:Failed to create endpoint"
...
spawnServer host tracer callback = withSocketsDo $ do
  etransport <- createTransport (defaultTCPAddr (unpack hostname) (show port)) defaultTCPParameters
  case etransport of
    Left e -> traceWith tracer (P2PServerDisconnected $ "Failed to create transport:" <> show e) >> fail "Failed to create transport"
    Right transport -> do
      eendpoint <- newEndPoint transport
      case eendpoint of
        Left e -> traceWith tracer (P2PServerDisconnected $ "Failed to create endpoint" <> show e) >> fail "Failed to create endpoint"
        Right endpoint -> do
          serverClosed <- newEmptyMVar
          withAsync (networkServer endpoint serverClosed) $ \serverHandle -> link serverHandle
 ...
 where
  networkServer endpoint serverDone =
   ...
   where
    ...
    go cs = do
      traceWith tracer (P2PServerConnected "Starting server async")
...
tryConnect n host tracer serverAddress =
  if n == 0
    then do
      traceWith tracer (P2PClientDisconnected "Client definitelly failed to connect!")
      pure $ Left ()
    else do
      etransport <- createTransport (defaultTCPAddr (unpack hostname) (show port)) defaultTCPParameters
      case etransport of
        Left e -> do
          traceWith tracer (P2PClientDisconnected $ "tryConnect: Failed to create transport" <> show e <> " address" <> show host <> ":" <> show port)
          pure $ Left ()
        Right transport -> do
          eendpoint <- newEndPoint transport
          case eendpoint of
            Left e -> do
              traceWith tracer (P2PClientDisconnected $ "tryConnect: Failed to create endpoint: " <> show e <> " address" <> show host <> ":" <> show port)
              pure $ Left ()
            Right endpoint -> do
              econn <- connect endpoint serverAddress ReliableOrdered defaultConnectHints
              case econn of
                Left _ -> do
                  traceWith tracer (P2PClientDisconnected $ "Client reconnecting..." <> show n)
                  threadDelay 1000000
...
connectClient host tracer newChannel serverAddress = withSocketsDo $ do
 ...
 where
  readAndSend conn chan = do
    ...
    case sendResult of
      Left e -> traceWith tracer (P2PClientConnected $ "Failed to send a message from the client:" <> show e) >> threadDelay 1000000 >> readAndSend conn chan
...
withP2PNetwork tracer localHost remoteHosts networkCallback between = withSocketsDo $ do
 ...
 where
  doIt = do
    ...
    spawnServer localHost tracer networkCallback
    connectAllClients bchan
