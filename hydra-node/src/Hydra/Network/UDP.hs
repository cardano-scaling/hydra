...
udpServer :: FromCBOR msg => Host -> NetworkCallback msg IO -> IO ()
udpServer Host{hostname, port} callback = do
  addrinfos <- getAddrInfo Nothing (Just $ unpack hostname) (Just $ show port)
  case addrinfos of
    (serveraddr : _) -> do
      sock <- socket (addrFamily serveraddr) Datagram defaultProtocol
      bind sock (addrAddress serveraddr)
      forever (receiveMsg sock callback)
    _ -> error "TODO: what if address is not resolved"

receiveMsg :: forall msg. FromCBOR msg => Socket -> NetworkCallback msg IO -> IO ()
receiveMsg sock callback = do
