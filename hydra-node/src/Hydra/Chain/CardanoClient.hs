...
awaitTransaction networkId socket tx =
 ...
 where
  ...
  go = do
    utxo <- foldMapM (queryUTxOByTxIn networkId socket QueryTip) ins
    if null utxo
      then go
      else pure utxo
...
queryUTxOByTxIn ::
  ...
  TxIn ->
  IO UTxO
queryUTxOByTxIn networkId socket queryPoint input =
  runQueryExpr networkId socket queryPoint $ do
    (AnyCardanoEra era) <- queryCurrentEraExpr
    (sbe :: ShelleyBasedEra e) <- liftIO $ assumeShelleyBasedEraOrThrow era
    eraUTxO <- queryInShelleyBasedEraExpr sbe $ QueryUTxO (QueryUTxOByTxIn (Set.fromList [input]))
    pure $ UTxO.fromApi eraUTxO
