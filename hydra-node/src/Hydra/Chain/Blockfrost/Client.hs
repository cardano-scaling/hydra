...
queryScriptRegistry txIds = do
  ...
  utxoList <- forM candidates $ queryUTxOByTxIn networkId
...
queryUTxOByTxIn networkId (TxIn txid _) = go (300 :: Int) (serialiseToRawBytesHexText txid)
 where
...
awaitTransaction tx = do
  ...
  let networkId = toCardanoNetworkId _genesisNetworkMagic
  foldMapM (queryUTxOByTxIn networkId) (txIns' tx)
