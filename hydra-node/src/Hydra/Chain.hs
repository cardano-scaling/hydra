...
instance (ArbitraryIsTx tx, Arbitrary (ChainStateType tx), HasAccumulatorElement tx, IsChainState tx) => Arbitrary (PostTxError tx) where
  arbitrary = genericArbitrary
...
instance (ArbitraryIsTx tx, Arbitrary (ChainStateType tx), HasAccumulatorElement tx, IsChainState tx) => Arbitrary (ChainEvent tx) where
  arbitrary = genericArbitrary
