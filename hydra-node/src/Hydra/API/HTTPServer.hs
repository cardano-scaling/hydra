...
httpApp tracer configDoc directChain env pparams getNodeState getCommitInfo getPendingDeposits putClientInput apiTransactionTimeout responseChannel request respond = do
  ...
  case (requestMethod request, pathInfo request) of
    ...
    ("POST", ["commit"]) ->
      consumeRequestBodyStrict request
        >>= handleDraftCommitUtxo tracer env pparams directChain getCommitInfo reqEnc respEnc
        >>= respond
    ("DELETE", ["commits", _]) ->
      consumeRequestBodyStrict request
...
handleDraftCommitUtxo ::
  ...
  LBS.ByteString ->
  IO Response
handleDraftCommitUtxo tracer env pparams directChain getCommitInfo reqEnc respEnc body = do
