					...
					\begin{walgo}{0.65}
						...
						\On{$(\hpRS,v,s,\underline{\tx}_{\mathsf{req}} ,\tx_\alpha,\tx_\omega)$ from $\party_j$}{
							...
							\Wait{$\hats = \bar{\mc S}.s ~ \land ~ v = \hatv$}{
								...
								$U_{\alpha}, U_{\omega} \gets \emptyset$ \;
								\If{$\tx_\omega \neq \bot$}{
								  ...
								  \If{$v = \bar{\mc S}.v \land \bar{\mc S}.U_{\omega} \neq \bot$}{
									\Req{$\bar{\mc S}.U_{\omega} = U_{\omega}$} \;
								  }
								  \Else{
									\Req{$\bar{\mc S}.U \applytx \tx_\omega \ne \bot$} \;
									$U_{\mathsf{active}} \gets \bar{\mc S}.U \applytx \tx_\omega \setminus U_{\omega}$ \;
								  }
								}
								\If{$\tx_\alpha \neq \bot$}{
								  $D \gets \mathcal{D}[\tx_\alpha]$ \;
								  \Req{$D.\mathsf{status} \neq \mathsf{Expired}$} \;
								  \Wait{$D.\mathsf{status} = \mathsf{Active}$}{
									...
									\If{$v = \bar{\mc S}.v \land \bar{\mc S}.U_{\alpha} \neq \bot$}{
									  \Req{$\bar{\mc S}.U_{\alpha} = D.U $} \;
									}
									$U_\alpha \gets D.U$ \;
									% NOTE: deposit is only spendable by the requested transactions;
									% it enters the local ledger state once incrementTx is observed
								...
								$\eta' \gets \accUTxO(\hatmU \cup U_{\alpha} \cup U_{\omega})$ \;
								${\eta'}^{\#} \gets (\eta')^{\#}$ \;
								${U_\omega}^{\#} \gets \hash(U_{\omega})$ \;
								${U_\alpha}^{\#} \gets \hash(U_{\alpha})$ \;
								$\msSig_i \gets \msSign(\hydraSigningKey, (\cid || v || \hats || {\eta'}^{\#} || {U_\omega}^{\#} || {U_\alpha}^{\#}))$ \;
						...
						\On{$(\hpAS,s,\msSig_j)$ from $\party_j$}{
							...
							\Wait{$\hats=s$}{
								...
								\If{$\forall k \in [1..n]: (k,\cdot) \in \hatSigma$}{
									...
									\Req{} $\msVfy(\hydraKeysAgg, (\cid || \hatv || \hats || {\eta'}^{\#} || {U_\omega}^{\#} || {U_\alpha}^{\#}), \msCSig)$ \;
			...
			\adjustbox{valign=t,scale=\sfact}{
				\begin{walgo}{0.6}
					...
					\On{$(\hpClose)$ from client}{
						...
						${U_\omega}^{\#} \gets \hash(\bar{\mc S}.U_\omega)$ \;
						${U_\alpha}^{\#} \gets \hash(\bar{\mc S}.U_\alpha)$ \;
						$\xi \gets \bar{\mc S}.\sigma$ \;
						% XXX: \hatv needed to distinguish between CloseType redeemer, explain how exactly?
						$\PostTx{}~(\mtxClose, \hatv, \bar{\mc S}.v, \bar{\mc S}.s, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#}, \xi)$ \;
					}
				\end{walgo}
			}
			\adjustbox{valign=t,scale=\sfact}{
				\begin{walgo}{0.6}
					...
					\On{$(\gcChainClose, {\eta'}^{\#}) \lor (\gcChainContest, s_{c}, {\eta'}^{\#})$ from chain}{
						\If{$\bar{\mc S}.s > s_{c}$}{
							...
							$\xi \gets \bar{\mc S}.\sigma$ \;
							% XXX: \hatv needed to distinguish between CloseType redeemer, explain how exactly?
							$\PostTx{}~(\mtxContest, \hatv, \bar{\mc S}.v, \bar{\mc S}.s, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#}, \xi)$ \;
						}
					}
				\end{walgo}
			}
