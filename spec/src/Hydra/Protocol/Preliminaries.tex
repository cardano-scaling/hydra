...
\subsection{KZG Accumulators}\label{sec:accumulators}

\noindent An accumulator scheme over a universe $\mathcal{U}$ is a set of algorithms where
\begin{itemize}
  \item $\accSetup$ generates public parameters (the common reference string),
  \item $\eta \gets \accUTxO(U)$ constructs an accumulator commitment from a UTxO set $U \subseteq \mathcal{U}$,
  \item $\eta' \gets \accCombine(\eta_1, \eta_2)$ combines two accumulator commitments into one,
  \item $\pi \gets \accWitness(\eta, S, U \setminus S)$ produces a membership witness $\pi$ proving that $S \subseteq U$,
  \item $\accVerify(\eta, S, \pi) \in \tyBool$ verifies a membership witness for subset $S$,
  \item $\eta' \gets \accExclude(\eta, S)$ removes subset $S$ from the accumulator, yielding the updated commitment $\eta'$, which itself serves as the exclusion witness, and
  \item $\accVerifyExclude(\eta, S, \eta') \in \tyBool$ verifies that $\eta'$ is the correct accumulator after removing $S$ from $\eta$.
\end{itemize}
...
The implementation uses KZG polynomial commitments~\cite{KZG10} over the BLS12-381 elliptic curve.

The common reference string produced by $\accSetup$ is not generated per head:
the protocol fixes a single \emph{canonical} CRS, obtained from the Ethereum KZG
ceremony (EIP-4844), whose G2 points are embedded in the protocol scripts at
compile time. On-chain validators receive the CRS via a reference input and
authenticate it by comparing the hash of its datum against the hash of the
canonical CRS embedded in the validator (see Section~\ref{sec:fanout-tx}).
