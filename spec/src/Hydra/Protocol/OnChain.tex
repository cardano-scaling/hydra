...
$\redeemerHead = (\mathsf{increment}, \xi, s, \txOutRef_{\mathsf{increment}}, {U_\omega}^{\#})$,
...
$\txOutRef_{\mathsf{deposit}}$ points to the claimed deposit and
${U_\omega}^{\#}$ is the hash of the snapshot's ordered decommit outputs (the
hash of the empty set if no decommit was pending). The validator
checks:
\begin{menumerate}
	...
	\[
		\msVfy(\hydraKeys,(\cid || v || s || {\eta'}^{\#} || {U_\omega}^{\#} || {U_\alpha}^{\#}),\xi) = \true
	\]
	...
	and ${U_\alpha}^{\#}$ is \emph{recomputed on-chain} from the deposited UTxOs $C$
	recorded in the datum of the claimed deposit input:
	\[
		{U_\alpha}^{\#} = \hash(\mathsf{concat}({\sortOn(1, C)}^{\downarrow2}))
	\]
	This binds the identity of the committed UTxO into the multi-signature: a
	deposit other than the one approved by the snapshot --- even one of equal
	value --- makes the signature verification fail.
...
\noindent The state-machine validator $\nuHead$ is spent with
$\redeemerHead = (\mathsf{decrement}, \xi, s, m, {U_\alpha}^{\#})$, where $\xi$ is a multi-signature of
...
${U_\alpha}^{\#}$ is the hash of the snapshot's ordered commit outputs (the
hash of the empty set if no deposit was pending). The
validator checks:
\begin{menumerate}
	\item State is advanced from $\datumHead \sim \stOpen$ to
	...
	and ${U_\omega}^{\#}$ is \emph{recomputed on-chain} as the hash of the ordered decommit
	outputs $o_{2} \dots o_{m+1}$ materialized by this transaction:
	\[
		{U_\omega}^{\#} = \hash(\bigoplus_{j=2}^{m+1} \bytes(o_{j}))
	\]
	This binds address, value and datum of every decommitted output into the
	multi-signature: redirecting the decommitted value to different outputs of
	equal total value makes the signature verification fail, so value
	conservation alone is no longer the only constraint on the decommit outputs.
...
\begin{menumerate}
	...
	\item Based on the redeemer $\mathsf{CloseType} = \mathsf{Initial} \cup (\mathsf{Any}, \xi, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#}) \cup (\mathsf{Unused}, \xi, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#}) \cup (\mathsf{Used}, \xi, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#})$, where $\xi$ is a multi-signature of the closing snapshot, ${\eta'}^{\#}$ is the hash of the unified accumulator commitment stored in the output datum, and ${U_\omega}^{\#}$ and ${U_\alpha}^{\#}$ are the hashes of the snapshot's ordered decommit and commit output sets (the hash of the empty set when none was pending), four cases are distinguished. In each case the closed state carries a single unified accumulator $\eta'$:
	\begin{menumerate}
		...
		\]
		\item $\mathsf{Any}$: Closing snapshot refers to current state version $v$ with no pending increments or decrements, and $s' > 0$. With no pending delta the unified accumulator degenerates to the snapshotted state:
		\[
			\eta' = \accUTxO(U')
		\]
		\[
		...
		\[
			{\eta'}^{\#} = (\eta')^{\#}
		\]
		\item $\mathsf{Unused}$: Closing snapshot refers to current state version $v$ and a pending increment or decrement is \emph{not} settled on-chain. The parties nevertheless signed the unified accumulator, which already includes the pending delta:
		\[
		...
		\[
			{\eta'}^{\#} = (\eta')^{\#}
		\]
		\item $\mathsf{Used}$: Closing snapshot refers to the previous state version $v - 1$ and a pending increment or decrement \emph{is} settled on-chain (which incremented the version to $v$). The unified accumulator combines the snapshotted UTxOs with the settled delta:
		\[
			\eta' = \accCombine(\accUTxO(U'), \eta_\Delta)
		\]
		\[
			\msVfy(\hydraKeys,(\cid || v - 1 || s' || {\eta'}^{\#} || {U_\omega}^{\#} || {U_\alpha}^{\#}),\xi) = \true
		\]
		\[
			{\eta'}^{\#} = (\eta')^{\#}
		\]
	...
	($U_\alpha$ or $U_\omega$; the accumulator of the empty set when nothing is
	pending). In every signed case the closed datum stores the same unified
	accumulator commitment $\eta'$ that the parties signed --- the case split
	only selects the version ($v$ or $v - 1$) against which the multi-signature
	is verified. Note that ${U_\omega}^{\#}$ and ${U_\alpha}^{\#}$ are supplied
	via the redeemer here, but they are bound by the multi-signature.

	The signed message is the concatenation of the six fields, each individually
	CBOR-encoded as Plutus \texttt{Data}, according to the CDDL schema:
	\begin{center}
		\begin{tabular}{ll}
			\texttt{headId = bytes .size 28}              & head identifier $\cid$                                                  \\
			\texttt{version = uint}                       & snapshot version $v$                                                    \\
			\texttt{number = uint}                        & snapshot number $s$                                                     \\
			\texttt{accumulatorHash = bytes .size 32}     & ${\eta'}^{\#}$, hash of the compressed unified accumulator commitment   \\
			\texttt{decommitOutputsHash = bytes .size 32} & ${U_\omega}^{\#}$, hash of the ordered decommit outputs                 \\
			\texttt{commitOutputsHash = bytes .size 32}   & ${U_\alpha}^{\#}$, hash of the ordered commit outputs                   \\
		\end{tabular}
	\end{center}
...
\begin{menumerate}
	...
	\[
		s' > s
	\]
	\item Based on the redeemer $\mathsf{ContestType} = (\mathsf{Unused}, \xi, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#}) \cup (\mathsf{Used}, \xi, {\eta'}^{\#}, {U_\omega}^{\#}, {U_\alpha}^{\#})$, where $\xi$ is a multi-signature of the contesting snapshot, ${\eta'}^{\#} = (\eta')^{\#}$ is the hash of the unified accumulator commitment stored in the output datum, and ${U_\omega}^{\#}$ and ${U_\alpha}^{\#}$ are the hashes of the snapshot's ordered decommit and commit output sets, two cases are distinguished:

	\begin{menumerate}
		\item $\mathsf{Unused}$: Contesting snapshot refers to current state version $v$ (pending delta not settled on-chain). The parties nevertheless signed the unified accumulator, which already includes the pending delta:
		\[
			\eta' = \accCombine(\accUTxO(U'), \eta_\Delta)
		\]
		\[
		...
		\[
			{\eta'}^{\#} = (\eta')^{\#}
		\]
		\item $\mathsf{Used}$: Contesting snapshot refers to the previous state version $v - 1$ (pending delta settled on-chain). The unified accumulator combines the snapshotted UTxOs with the settled delta:
		\[
			\eta' = \accCombine(\accUTxO(U'), \eta_\Delta)
		\]
		\[
		...
		\[
			{\eta'}^{\#} = (\eta')^{\#}
		\]
		where $\eta_\Delta$ is the accumulator commitment of the pending delta
		UTxOs (the accumulator of the empty set when nothing is pending). As for
		\mtxClose{}, in both cases the closed datum stores the unified
		accumulator the parties signed and the case split only selects the
		version against which the multi-signature is verified. The signed
		message is constructed as defined in Section~\ref{sec:close-tx}.
	\end{menumerate}
...
\end{itemize}
The head validator embeds the hash ${\mathsf{CRS}}^{\#}_{\mathsf{canonical}}$ of
the \emph{canonical} CRS, computed over its (compressed) G2 points. The
canonical CRS is fixed when the protocol scripts are compiled and stems from the
Ethereum KZG ceremony. Since fanning out is permissionless --- no participant
signature is required --- the CRS provided via the reference input must be
authenticated by \emph{content}: binding only the location (address or
reference script) of the reference input would allow an attacker to publish and
substitute a setup with known trapdoor $\tau$ and forge membership proofs for
arbitrary outputs, stealing all funds in the head.
...
\begin{menumerate}
	...
	\]
	\item The reference input pointed to by $\mathsf{crsRef}$ carries a CRS datum $\mathsf{crs}$ matching the canonical CRS embedded in the validator:
	\[
		{\mathsf{crs}}^{\#} = {\mathsf{CRS}}^{\#}_{\mathsf{canonical}}
	\]
...
Note that the unified accumulator $\eta$ may contain \emph{pre-settled}
elements, which are no longer backed by head value at fanout time: outputs of a
pending decommit that was settled on-chain before closing (they were already
paid out by the \mtxDecrement{} transaction), or outputs of a pending deposit
that was never incremented (they never entered the head and are instead
recoverable through the deposit validator, see
Section~\ref{sec:recover-tx}). Such elements are simply not distributed:
membership verification proves that the distributed outputs are committed under
$\eta$, while the strict value conservation forces the entire head value to be
paid to exactly those outputs. Pre-settled elements remain in the accumulator
undistributed; likewise for partial fanout below, exclusion witnesses only ever
remove distributed batches and the final membership check ignores leftover
pre-settled elements.
...
\begin{menumerate}
  ...
  \item The reference input pointed to by $\mathsf{crsRef}$ carries a CRS datum $\mathsf{crs}$ matching the canonical CRS embedded in the validator: ${\mathsf{crs}}^{\#} = {\mathsf{CRS}}^{\#}_{\mathsf{canonical}}$.
