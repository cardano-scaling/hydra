...
\begin{menumerate}
	...
	\]
	\item This deposit is the only one spent by the transaction. The recovered
	outputs $o_{1} \dots o_{m}$ are the transaction's first $m$ outputs and are
	therefore shared by every deposit input, so two deposits whose $C$ hashes
	alike would both be satisfied by a single set of recovered outputs, leaving
	the second deposit's value to be directed anywhere by the transaction author.
	Note that $\nuDeposit$ neither parses the bytes of $C$ nor relates them to the
	value the deposit holds, so hashing alike is weaker than being equal:
	\[
		|\{ (\cdot, o) \in \txInputs \mid o \text{ is governed by } \nuDeposit \}| = 1
	\]
\end{menumerate}
...
\begin{menumerate}
	...
	\]
	\item The head input is spent with an $\mathsf{increment}$ redeemer that
	claims this very deposit. Otherwise a legitimate \mtxIncrement{} could carry
	additional deposit inputs whose value enters the head without any snapshot
	crediting it:
	\[
...
\begin{menumerate}
	...
	\[
		\txOutRef_{\mathsf{increment}} = \txOutRef_{\mathsf{deposit}}
	\]
	\item The claimed deposit is the first output of its transaction, as produced
	by \mtxDeposit{}~\ref{sec:deposit-tx}. Since $\eta_{\alpha}$ below binds a
	deposit by its transaction id, sibling outputs of that same transaction would
	otherwise be interchangeable under one signature:
	\[
		\txIdx(\txOutRef_{\mathsf{deposit}}) = 0
	\]
	...
	The commit digest $\eta_{\alpha}$ is recomputed from the claimed deposit itself,
	binding both its recorded outputs $C$ and the identity of the transaction that
	created it:
	\[
		\eta_{\alpha} = \hash(\hash(\mathsf{concat}({\sortOn(1, C)}^{\downarrow2})) || \mathsf{txId}(\txOutRef_{\mathsf{deposit}}))
	\]
...
\begin{menumerate}
	...
	\[
		v' = v + 1
	\]
	\item At least one decommit output is materialized. Note that this is checked on
	...
	signed message. With no decommit outputs, $\eta_{\omega}$ recomputed here is the
	digest of the empty set — exactly what a snapshot carrying no pending decommit
	produces — while $\eta_{\alpha}$ comes from the redeemer. An
	\mtxIncrement{} snapshot's multi-signature would otherwise authorize a decrement
	that advances the version and adopts that snapshot's accumulator, which already
	counts the deposited UTxOs, while no deposit is spent and no value enters the
	head. The head would then account for UTxOs it does not hold.
