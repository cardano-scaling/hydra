...
\begin{itemize}
	...
	\item $\hatmU$: UTxO set of the last requested (seen) snapshot, from which
	      the local ledger state is rebuilt.
	...
	\item $\bar{\mc S}$: Snapshot object of the latest confirmed snapshot which
	      ...
	      \begin{center}
		      \begin{tabular}{|l|l|}\hline
			      ...
			      $\bar{\mc S}.{\eta'}^{\#}$       & hash of the unified accumulator commitment over $U \cup U_\alpha \cup U_\omega$ \\ \hline
		      \end{tabular}
	      \end{center}
\end{itemize}
...
\begin{enumerate}
	...
	\item If a deposit is requested: verify the corresponding deposit is
	      $\text{Active}$ and add the deposited UTxO to the active UTxO set used
	      to validate the requested transactions. The deposited UTxO is recorded
	      as pending $U_{\alpha}$ but \emph{not} added to the local ledger
	      state: it becomes spendable on L2 only once the corresponding
	      $\mathtt{incrementTx}$ is observed on-chain. Otherwise the same
	      deposit would be re-injected into the spendable UTxO set on every
	      snapshot round and could be spent once per snapshot, inflating the L2
	      balance.
...
snapshot as
$\hatmU \gets (U_{\mathsf{active}} \applytx \underline{\tx}_{\mathsf{req}}) \setminus U_{\alpha}$
--- a pending deposit is part of the active set only to validate the requested
transactions, it is not part of the snapshotted UTxO set. Then, $\party_i$
creates a signature $\msSig_i$ using their signing key $\hydraSigningKey$ on the
message
\[
	\cid || v || \hats || {\eta'}^{\#} || {U_\omega}^{\#} || {U_\alpha}^{\#}
\]
where ${\eta'}^{\#}$ is the hash of the unified accumulator commitment
$\eta' = \accUTxO(\hatmU \cup U_{\alpha} \cup U_{\omega})$ over the snapshotted
UTxO set and any pending commit or decommit UTxOs, and ${U_\omega}^{\#}$ and
${U_\alpha}^{\#}$ are the hashes of the ordered decommit outputs
$U_{\omega} = \mathsf{outputs}(\tx_{\omega})$ and the deposited $U_{\alpha}$
respectively (the hash of the empty set when no decommit or deposit is
pending). Binding ${U_\omega}^{\#}$ and ${U_\alpha}^{\#}$ into the signature
allows the \mtxDecrement{} and \mtxIncrement{} validators to recompute them
from the materialized L1 outputs and the claimed deposit (see
Sections~\ref{sec:decrement-tx} and~\ref{sec:increment-tx}). The signature is
sent to all head members via message $(\hpAS,\hats,\msSig_i)$. Finally, the
local ledger state $\hatmL$ and pending transaction set $\hatmT$ get pruned by
re-applying all locally pending transactions $\hatmT$ to the just requested
snapshot's UTxO set $\hatmU$ iteratively and ultimately yielding a ``pruned''
version of $\hatmT$ and $\hatmL$. \\
...
state $U_{\alpha}$ is cleared. This is the point at which the deposited UTxO
becomes spendable on L2 (see $\hpRS$ above). Also the observed version $v$ is
used for future snapshots by setting $\hatv = v$. Note that the version of the
open head state is incremented on each \mtxIncrement{} transaction as described
in Section~\ref{sec:increment-tx}
