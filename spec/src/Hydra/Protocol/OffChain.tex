	...
	\item $\bar{\mc S}$: Snapshot object of the latest confirmed snapshot which
	      ...
	      \begin{center}
		      \begin{tabular}{|l|l|}\hline
			      ...
			      $\bar{\mc S}.\txOutRef_\alpha$ & deposit the pending UTxO to increment comes from    \\ \hline
...
\begin{enumerate}
	...
	\item If a decommit is requested: verify it produces at least one output,
...
$\tx_{\omega}$ respectively. Note that $\eta_{\alpha}$ covers both $U_{\alpha}$
and the deposit $\txOutRef_\alpha$ it comes from (see
Section~\ref{sec:increment-tx}), so the resulting signature authorizes that one
deposit and no other recording the same UTxO. The signature is sent to all head members via
