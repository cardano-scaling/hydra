...
// fletcher package (pinned from nixpkgs via the wrapped typst; see
// nix/hydra/spec.nix). The head state machine is rendered from
// `head-fsm-transitions`, the single data source that check-refs.sh
// cross-checks against the Agda `_⟶⟨_⟩_` relation.
