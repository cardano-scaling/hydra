...
\dparagraph{$\hpRD$.}\quad Upon receiving request $(\hpRD,\tx_\omega)$, \red{if
  ...
  retry later or eventually marks the decommit as invalid. \red{After applying $\tx$, it's inputs
...
\red{\dparagraph{$\hpRI$.}\quad Upon receiving request $(\hpRI,\tx)$, if
  another decrement (or increment) is pending to be settled the transaction is
  rejected. Otherwise the transaction inputs are kept in the local state ($U_\alpha$) to be included
...
\red{\item
    there is a pending increment $\tx_\alpha$ which inputs will be made available in the active UTxO set
	$U_{\mathsf{active}}$}
\end{itemize}
...
\todo{FIXME: race-condition on RecInc/RecDec requests; use seen snapshot to resolve}
