...
gets recorded in $\hatmT$ and applied to the local \emph{seen} ledger state
...
snapshot is confirmed ($\bars = \hats$), all requested transactions
$\mT_{res}$ are applicable to $\barmU$, and all requested transactions have
been seen ($\mT_{res} \subseteq \hatmT$).
