...
singlePartyCommitsScriptBlueprint tracer workDir backend hydraScriptsTxId =
 ...
 where
  prepareScriptPayload lovelaceAmt = do
    ...
    (scriptIn, scriptOut) <- createOutputAtAddress backend scriptAddress datumHash (lovelaceToValue lovelaceAmt)
...
canCommit :: ChainBackend backend => Tracer IO EndToEndLog -> FilePath -> backend -> [TxId] -> IO ()
canCommit tracer workDir backend hydraScriptsTxId =
  (`finally` returnFundsToFaucet tracer backend Alice) $ do
