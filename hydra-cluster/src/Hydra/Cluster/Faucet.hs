...
seedFromFaucet backend receivingVerificationKey lovelace tracer = do
 ...
 where
  submitSeedTx faucetVk faucetSk networkId = do
    faucetUTxO <- findFaucetUTxO backend lovelace
...
findFaucetUTxO :: ChainBackend backend => backend -> Coin -> IO UTxO
findFaucetUTxO backend lovelace = do
  -- TODO: the call to get networkId is duplicated above, we should take it as the argument here
  networkId <- Backend.queryNetworkId backend
...
createOutputAtAddress backend atAddress datum val = do
  (faucetVk, faucetSk) <- keysFor Faucet
  utxo <- findFaucetUTxO backend 0
