...
import HydraNode (allocateHydraNodePortsFor, getSnapshotUTxO, input, output, prepareHydraNode, requestCommitTx, send, waitFor, waitForAllMatch, waitForNodesConnected, waitForNodesSynced, waitMatch, withHydraCluster, withHydraNode, withPreparedHydraNode, withSoloHydraNode, withUnsyncedSoloHydraNode)
...
fanoutWithNOutputs numOutputs tmpDir tracer hydraScriptsTxId opts = do
  ...
  withHydraCluster hydraTracer timing tmpDir nodeSocket' 1 [(aliceCardanoVk, mkSecret aliceCardanoSk)] [aliceSk] hydraScriptsTxId $ \nodes -> do
    ...
    -- Deposit UTxOs into the head
    (aliceExternalVk, aliceExternalSk) <- generate genKeyPair
    committedUTxOByAlice <- seedFromFaucet opts aliceExternalVk (lovelaceToValue (fromInteger $ numOutputs * 1_000_000)) (contramap FromFaucet tracer)
