...
filterSnapshotConfirmedByAddressScenario tracer tmpDir = do
  scenarioSetup tracer tmpDir $ \backend nodes hydraTracer -> do
...
scenarioSetup ::
  ...
  FilePath ->
  (DirectBackend -> NonEmpty HydraClient -> Tracer IO HydraNodeLog -> IO a) ->
  IO a
scenarioSetup tracer tmpDir action = do
  withCardanoNodeDevnet (contramap FromCardanoNode tracer) tmpDir $ \backend -> do
    ...
    withHydraCluster hydraTracer tmpDir nodeSocket' firstNodeId cardanoKeys hydraKeys hydraScriptsTxId contestationPeriod $ \nodes -> do
      ...
      action backend nodes hydraTracer
