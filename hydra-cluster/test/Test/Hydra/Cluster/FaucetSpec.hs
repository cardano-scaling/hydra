...
import CardanoNode (EndToEndLog (..), runBackend, withCardanoNodeDevnet)
...
setupDevnet :: ((Tracer IO FaucetLog, ChainBackendOptions) -> IO a) -> IO a
setupDevnet action =
  failAfter 30 $
    showLogsOnFailure "FaucetSpec" $ \tracer ->
      withTempDir "hydra-cluster" $ \tmpDir ->
        withCardanoNodeDevnet (contramap FromCardanoNode tracer) tmpDir $ \_ directOpts ->
          action (contramap FromFaucet tracer, Direct directOpts)
...
spec =
  around setupDevnet $ do
    ...
    describe "returnFundsToFaucet" $ do
      ...
      it "seedFromFaucet and returnFundsToFaucet should work together" $ \(tracer, opts) -> do
        withMaxSuccess 10 $
          forAll (Coin <$> choose (1000000, 10000000000)) $ \coin ->
            forAll (elements [Alice, Bob, Carol]) $ \actor -> do
              (vk, _) <- keysFor actor
              (faucetVk, _) <- keysFor Faucet
              initialFaucetFunds <- runBackend opts $ queryUTxOFor QueryTip faucetVk
              void $ seedFromFaucet opts vk (lovelaceToValue coin) tracer
              returnFundsToFaucet tracer opts actor
              remaining <- runBackend opts $ queryUTxOFor QueryTip vk
    ...
    describe "publishHydraScriptsAs" $ do
      it "squash the UTxO to get a suitable output" $ \(tracer, opts) -> do
        -- NOTE: Note use 'Faucet' as this has a very big initial amount
        (vk, _) <- keysFor Alice
        -- NOTE: 88 ADA is just enough to pay for reference scripts deposits.
        forM_ [1_000_000, 2_000_000, 88_000_000] $ \c -> seedFromFaucet opts vk (lovelaceToValue c) tracer

        void $ publishHydraScriptsAs opts Alice

        -- it squashed the UTxO
        utxoAfter <- runBackend opts $ queryUTxOFor QueryTip vk
