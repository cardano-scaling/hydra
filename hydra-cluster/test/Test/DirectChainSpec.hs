...
import Hydra.Chain.Direct (IntersectionNotFoundException (..), runDirectBackend, withDirectChain)
...
spec = around (showLogsOnFailure "DirectChainSpec") $ do
  it "can open a head" $ \tracer -> do
    withTempDir "hydra-cluster" $ \tmp -> do
      withCardanoNodeDevnet (contramap FromNode tracer) tmp $ \blockTime directOpts -> do
        ...
        -- Alice setup
        aliceChainConfig <- chainConfigFor Alice tmp (Direct directOpts) hydraScriptsTxId [Bob, Carol] timing
        withDirectChainTest (contramap (FromDirectChain "alice") tracer) aliceChainConfig alice $ \aliceChain@CardanoChainTest{postTx} -> do
          -- Bob setup
          bobChainConfig <- chainConfigFor Bob tmp (Direct directOpts) hydraScriptsTxId [Alice, Carol] timing
          withDirectChainTest nullTracer bobChainConfig bob $ \bobChain@CardanoChainTest{} -> do
            ...
            let headParameters = HeadParameters cperiod dperiod [alice, bob, carol]
  ...
  it "can open, close & fanout a Head" $ \tracer -> do
    withTempDir "hydra-cluster" $ \tmp -> do
      withCardanoNodeDevnet (contramap FromNode tracer) tmp $ \blockTime directOpts -> do
        ...
        (aliceCardanoVk, _) <- keysFor Alice
        seedFromFaucet_ (Direct directOpts) aliceCardanoVk 100_000_000 (contramap FromFaucet tracer)
        aliceChainConfig <- chainConfigFor Alice tmp (Direct directOpts) hydraScriptsTxId [] timing
        withDirectChainTest (contramap (FromDirectChain "alice") tracer) aliceChainConfig alice $
          \aliceChain@CardanoChainTest{postTx} -> do
            ...
            failAfter 5 $
              waitForUTxO (Direct directOpts) expectedUTxO
    ...
    withTempDir "hydra-cluster" $ \tmp -> do
      withCardanoNodeDevnet (contramap FromNode tracer) tmp $ \blockTime directOpts -> do
        ...
        tip <- withDirectChainTest (contramap (FromDirectChain "alice") tracer) aliceChainConfig alice $
          \aliceChain@CardanoChainTest{postTx} -> do
            tip <- runBackend (Direct directOpts) queryTip
            postTx $ InitTx{participants, headParameters}
  ...
  it "cannot restart head to an unknown point" $ \tracer -> do
    withTempDir "hydra-cluster" $ \tmp -> do
      withCardanoNodeDevnet (contramap FromNode tracer) tmp $ \blockTime directOpts -> do
        ...
        aliceChainConfig <-
          chainConfigFor Alice tmp (Direct directOpts) hydraScriptsTxId [] timing
  ...
  it "can publish and query reference scripts in a timely manner" $ \tracer -> do
    withTempDir "hydra-cluster" $ \tmp -> do
      withCardanoNodeDevnet (contramap FromNode tracer) tmp $ \_blockTime directOpts -> do
        ...
        failAfter 5 $ void $ runBackend (Direct directOpts) (queryScriptRegistry hydraScriptsTxId)
...
withDirectChainTest tracer config party action = do
  ...
  ctx <- runDirectBackend directOptions $ loadChainContext configuration party
  eventMVar <- newLabelledEmptyTMVarIO "direct-chain-events"

  let callback event = atomically $ putTMVar eventMVar event

  wallet <- mkTinyWallet (runDirectBackend directOptions) tracer configuration
  withDirectChain directOptions tracer configuration ctx wallet (initHistory initialChainState) callback $ \Chain{postTx} -> do
    action
      CardanoChainTest
        { postTx
